...
<main class="main-content">
    ...
    <form action="{{ route('noticias.store') }}" method="POST" enctype="multipart/form-data" class="form-news">
        @csrf
        ...
        <div class="form-group">
            ...
            <textarea name="contenido" id="contenido" rows="5" 
                      class="@error('contenido') is-invalid @enderror" required>{{ old('contenido') }}</textarea>
            @error('contenido')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
